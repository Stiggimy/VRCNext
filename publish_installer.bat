...
:: ── 0b. Patch version in AppInfo.cs ─────────────────────────────────────────
powershell -NoProfile -Command "$a='%~dp0AppInfo.cs'; $q=[char]34; $c=[IO.File]::ReadAllText($a); $c=$c -replace ('Version = '+$q+'[^'+$q+']+'+$q),('Version = '+$q+'%VERSION%'+$q); [IO.File]::WriteAllText($a,$c,[Text.Encoding]::UTF8); Write-Host ' [OK] AppInfo.cs version  -> %VERSION%'"
echo.
